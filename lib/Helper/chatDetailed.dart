...
class ChatDetailed extends StatefulWidget {
  ...
  ChatDetailed({required this.userData});
  @override
...
class _ChatDetailedState extends State<ChatDetailed> {
 ...
 String chatId="";
              ...
              ? ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (snapshot.data!.docs.length == 1)
                      return Column(
                        children: [
                          _timeDivider((message.data() as dynamic)['time']),
                          _messageItem(message, context),
                        ],
                      );
                    if (index == 0) {
                      past = (message.data() as dynamic)['time'];
                    ...
                    if (index == snapshot.data!.docs.length - 1)
                      return Column(
                        children: [
                          ...
                          _messageItem(message, context),
                          if (!sameDay(
                              toPass, (message.data() as dynamic)['time']))
                            _timeDivider(toPass),
                        ],
                      );
                    past = (message.data() as dynamic)['time'];
                    return sameDay((message.data() as dynamic)['time'], toPass)
                        ? _messageItem(message, context)
                        : Column(
                            children: [
                              _messageItem(message, context),
  ...
  _messageItem(DocumentSnapshot message, BuildContext context) {
    final bool isMe = (message.data() as dynamic)['from'] == myId;
    Timestamp time = (message.data() as dynamic)['time'];
    ...
    if ((message.data() as dynamic)['isText'])
      return Align(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                (message.data() as dynamic)['message'].toString(),
                style: TextStyle(
                  color: isMe
    ...
    //   future: dbHelper.getURLforImage((message.data()as dynamic)['photo'].toString()),
    //   builder: (context, snapshot) {
    //     return Align(
    //       alignment: isMe ? Alignment.centerRight : Alignment.centerLeft,
    //       child: Container(
